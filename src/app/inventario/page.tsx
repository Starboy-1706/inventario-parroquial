...
export async function generateMetadata() {
  return authPageMetadata("Inventario");
}
...
export default async function InventarioPage({
  ...
  const activeZone = sp.zona
    ? allZones.find((z) => z.id === Number(sp.zona))
    : undefined;
    ...
    <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
      <header className="animate-fade-up">
        ...
        <h1 className="mt-3 font-display text-4xl font-semibold tracking-tight text-ink sm:text-5xl">
          {activeZone ? (
            <>
              Zona: <em className="italic text-gold-deep">{activeZone.name}</em>
            </>
          ) : (
            "Inventario"
          )}
        ...
        <p className="mt-2 text-sm text-ink-soft">
          ...
          {sp.estado ? "" : " en inventario"} · pulsa cualquier ficha para ver su
          detalle y código QR
        </p>
      </header>
      ...
      <section className="mt-6 animate-fade-up" style={{ animationDelay: "200ms" }}>
        {items.length === 0 ? (
          <EmptyState
            ...
            description="No hay artículos que coincidan con los filtros actuales. Prueba a quitar algún filtro o da de alta un nuevo artículo."
          />
        ) : (
              ...
              return (
                <li key={it.id} className={cn(i !== 0 && "border-t border-line-soft")}>
                  <Link
                    ...
                    className="group relative flex items-center gap-3.5 px-4 py-3.5 transition-colors hover:bg-paper/60 sm:gap-5 sm:px-5"
                  >
                    <span
                    ...
                    {it.photoId ? (
                      <span className="relative h-11 w-11 shrink-0 overflow-hidden rounded-xl border border-line sm:h-12 sm:w-12">
                        ...
                        <img
                          src={photoUrl(it.photoId) ?? ""}
                          alt=""
                          className="h-full w-full object-cover"
                        />
                      </span>
                    ) : null}
                    <span className="hidden shrink-0 rounded-lg border border-line bg-white px-2.5 py-1.5 font-mono text-[0.7rem] font-semibold tracking-wide text-ink-soft sm:block">
                      {it.code}
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold text-ink sm:text-[0.95rem]">
                        {it.name}
                      </p>
                      <p className="mt-0.5 truncate text-xs text-ink-soft">
                        <span className="font-mono text-[0.68rem] sm:hidden">
                          {it.code} ·{" "}
                        </span>
                        {it.zoneName}
                    ...
                    <div className="shrink-0 text-right">
                      {it.itemType === "CONTABLE" ? (
                        <>
                          ...
                          >
                            {it.quantity}{" "}
                            <span className="font-sans text-[0.65rem] font-medium text-ink-faint">
                              uds.
                            </span>
                          </p>
                          {lowStock && (
                            <p className="mt-1 text-[0.62rem] font-bold uppercase tracking-wide text-red-600">
                              stock bajo
                            </p>
                          )}
                        </>
